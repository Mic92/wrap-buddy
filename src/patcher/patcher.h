...
#include "stub_64.h"
#include "stub_64_reloc.h"
#ifdef HAVE_STUB_32
#include "stub_32.h"
#include "stub_32_reloc.h"
#endif
...
inline auto get_stub(bool is_64bit, bool is_reloc) -> std::span<const uint8_t> {
  if (is_64bit) {
#if defined(__x86_64__) || defined(__aarch64__)
    // On 64-bit platforms, stub_bin is 64-bit
    if (is_reloc) {
      return std::span<const uint8_t>{stub_reloc_bin};
    } else {
  ...
  if (is_reloc) {
    return std::span<const uint8_t>{stub32_reloc_bin};
  } else {
...
auto find_entry_segment(std::span<const uint8_t> data, uint64_t entry_vaddr)
  ...
  const auto phdr_span = subspan_checked(data, ehdr->e_phoff, phdr_table_size);
  ...
  for (const auto &phdr : phdrs) {
    if (phdr.p_type == PT_LOAD && entry_vaddr >= phdr.p_vaddr &&
      ...
      // Validate segment is within file bounds using checked access
      const auto segment = subspan_checked(data, file_offset, available_space);
      if (segment.size() < available_space) {
        return std::nullopt;
      }
...
// Figure out the loader filename. Assumes that get_stub is successful.
inline auto get_loader_filename([[maybe_unused]] bool is_64bit)
    -> std::string_view {
#if defined(__x86_64__) || defined(__aarch64__)
  return is_64bit ? "loader.bin" : "loader32.bin";
#else
  return "loader.bin";
#endif
}

inline auto patch_binary(const fs::path &binary_path, fs::path interp_path,
                         const std::string &rpath, bool dry_run,
                         const std::vector<std::string> &needed,
                         bool relocatable, const fs::path &loader_dir_path)
    -> Result<> {
  ...
  const auto elf_class = get_at(data, EI_CLASS);
  if (!elf_class) {
    return std::unexpected(std::format("'{}' is too small to read ELF class",
                                       binary_path.string()));
  }
  ...
  auto stub = get_stub(is_64bit, relocatable);
  ...
  // Rewrite the placeholder path in relocatable mode.
  std::string rewritten_stub;
  if (relocatable) {
    fs::path loader_path = loader_dir_path / get_loader_filename(is_64bit);

    auto loader_rel_path = fs::relative(loader_path, binary_path.parent_path());

    if (loader_rel_path.native().size() >=
        sizeof(RELOCATABLE_LOADER_PATH_PLACEHOLDER)) {
      return std::unexpected(
          std::format("relative path to loader '{}' is longer than the stub "
                      "placeholder path",
                      loader_rel_path.string()));
    }

    rewritten_stub =
        std::string(reinterpret_cast<const char *>(stub.data()), stub.size());

    auto placeholder_pos =
        rewritten_stub.find(RELOCATABLE_LOADER_PATH_PLACEHOLDER);
    if (placeholder_pos == rewritten_stub.npos) {
      return std::unexpected("no loader path placeholder in the stub");
    }

    auto rewritten_placeholder_path = loader_rel_path.string();

    /* Fill with zeros for NUL-termination. */
    rewritten_placeholder_path.resize(
        sizeof(RELOCATABLE_LOADER_PATH_PLACEHOLDER));

    rewritten_stub.replace(placeholder_pos, rewritten_placeholder_path.size(),
                           rewritten_placeholder_path);

    stub = std::span<const uint8_t>{
        reinterpret_cast<const uint8_t *>(rewritten_stub.data()),
        rewritten_stub.size(),
    };
  }
  ...
  // Save original bytes for config file using checked access
  const auto file_offset = entry_info->file_offset;
  const auto orig_span = subspan_checked(data, file_offset, padded_size);
  if (orig_span.size() < padded_size) {
    return std::unexpected("entry point region extends past end of file");
  }
  std::vector<uint8_t> orig_bytes(orig_span.begin(), orig_span.end());
  ...
  if (auto err =
          write_config_file(config_path, is_64bit, orig_entry, padded_size,
                            interp_path.string(), rpath, orig_bytes, needed);
      !err) {
    return std::unexpected(err.error());
  }
  std::println("  Wrote config to {}", config_path.string());

  // Get mutable view for in-place patching using checked access
  auto mutable_data = mem.mutable_data();
  auto patch_region = subspan_checked(mutable_data, file_offset, padded_size);
  if (patch_region.size() < padded_size) {
    return std::unexpected("patch region extends past end of file");
  }

  // Overwrite entry point with stub (padded with zeros)
  std::ranges::copy(stub, patch_region.begin());
  std::ranges::fill(patch_region.subspan(stub.size()), uint8_t{0});
  ...
  if (interp_offset) {
    auto interp_span =
        subspan_checked(mutable_data, *interp_offset, sizeof(uint32_t));
    if (interp_span.size() < sizeof(uint32_t)) {
      return std::unexpected("PT_INTERP offset is outside file bounds");
    }
    // p_type is always first field and always 4 bytes
    *start_lifetime_as<uint32_t>(interp_span.data()) = PT_NULL;
